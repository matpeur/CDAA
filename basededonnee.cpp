...
void Basededonnee::AjoutContact(Contact P)
{
    ...
    if(db.open())
    {
     ...
     QSqlQuery query;
     query.prepare("INSERT INTO conta(nom,prenom,entreprise,mail,telephone,photo,date)" "VALUES(:A,:B,:C,:D,:E,:F,:G)");
     ...
     query.bindValue(":G",QString::fromStdString(P.getTelephone()));
...
void Basededonnee::ModifiContact(Contact old, Contact C)
{
    ...
    query.bindValue(":G",QString::fromStdString(C.getTelephone()));
    ...
    query.bindValue(":g",QString::fromStdString(old.getTelephone()));
